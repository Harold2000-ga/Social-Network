...
const unFollow = (req, res) => {
  ...
  Follow.findOneAndRemove({ user: userId, followed: followedId })
    .exec()
    .then(item => {
      if (!item) {
...
const following = (req, res) => {
  ...
  Follow.paginate(
    { user: userId },
    {
      ...
      limit: itemPerPage,
      sort: '_id',
      populate: { path: 'followed', select: '-role -bio -password -email' },
    }
  )
    .then(async result => {
      if (!result) {
...
const followers = (req, res) => {
  ...
  Follow.paginate(
    { followed: userId },
    {
      ...
      limit: itemPerPage,
      sort: '_id',
      populate: { path: 'user', select: '-role -bio -password -email' },
    }
  )
    .then(async result => {
      if (!result) {
